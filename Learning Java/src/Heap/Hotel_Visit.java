...
public class Hotel_Visit {
	...
	public static void main(String[] args) {
		...
		while (Q-- > 0) {
			int q = scn.nextInt();
			if (q == 1) {
				addDistance(scn.nextLong(), scn.nextLong(), k);
				continue;
			}
			if (q == 2)
				System.out.println(getKth(k - 1));
		}
	...
	public static void addDistance(long x, long y, int k) {
		long dist = x * x + y * y;
		if (queue.size() < k)
			queue.add(dist);
	...
	public static long getKth(int k) {
		return queue.peek();
	}
