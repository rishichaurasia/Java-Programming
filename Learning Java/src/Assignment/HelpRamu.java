 ...
 */
		...
		while(t-->0) {
			...
			for(int i = 0; i<m; i++)
				bi[i] = scn.nextInt();
			minCost(c1, c2, c3, c4, ai, bi);
		}
		
		
	}
	
	public static void minCost(int c1, int c2, int c3, int c4, int[] ai, int[] bi) {
		int sum1 = 0;
		for (int i = 0; i < ai.length; i++) {
			sum1 += Math.min((ai[i]*c1),c2);
		}
		sum1 = Math.min(sum1, c3);
		int sum2 = 0;
		for (int i = 0; i < bi.length; i++) {
			sum2 += Math.min((bi[i]*c1),c2);
		}
		sum2 = Math.min(sum2, c3);
		int sum = Math.min(sum1+sum2, c4);
