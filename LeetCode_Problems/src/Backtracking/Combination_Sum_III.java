...
//		combinationSum3a(k, 1, n, list, new ArrayList<>());
		return list;
	}
	...
	private static void combinationSum3a(int k, int start, int n, List<List<Integer>> list, ArrayList temp) {
		if (k == 0 && n == 0) {
			list.add(new ArrayList<>(temp));
			return;
		}
		if ((k > 9 - start + 1) || (n < 0))
			return;
		temp.add(start);
		combinationSum3a(k - 1, start + 1, n - start, list, temp);
		temp.remove(temp.size() - 1);
		combinationSum3a(k, start + 1, n, list, temp);
